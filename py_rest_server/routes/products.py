...
from py_rest_server.models.product_model import Product
...
async def read_products_by_type(product_type: int):
    items = await get_products_by_type(product_type)
    if items:
        return [Product(**item) for item in items]
    raise HTTPException(status_code=404, detail="Products not found for given type")
